...
{//Inicio CLASS ATV2
  ...
  {//Inicio MAIN
     ...
     while(!str.equals("FIM"))
     {//Inicio WHILE
        help = Double.parseDouble(str);
        lista.pesquisaSequen(help, x);
        str = MyIO.readLine();
     }//Fim WHILE
     long tempoFim = now();
     Arq.openWrite("550274_binaria.txt");
     Arq.println("550274\t"+(tempoFim-tempoInicio)/1000.0 + " s." + "\t"+"numero comparacao");
     Arq.close();
  }// Fim MAIN
}//Fim CLASS ATV2
...
class Lista
{
  ...
  public void inserir (Municipio muni) 
  {
     ...
     {
        //MyIO.println("Inserir (ultimo + 1) % lista.length) == primeiro ====> "+ultimo+" "+primeiro);
        remover();
        lista[ultimo] = muni; 
  ...
  public void remover () 
  {
     ...
     if (primeiro == ultimo)
     {
        //MyIO.println("Remover ultimo == primeiro ====> "+ultimo+" "+primeiro);
  ...
  public void ordenar(int n)
  {
     ...
     {//Inicio FOR
        for(int j = i+1; j < n; j++)
        {//Inicio FOR2
           if(lista[i].getTemperatura() > lista[j].getTemperatura())
           {//Inicio IF
              aux = lista[j];
              lista[j] = lista[i];
              lista[i] = aux;
  ...
  public void pesquisaSequen(double x, int fim)
  {
     int meio;
     int inicio = 0;
     while (inicio <= fim) 
     { //Inicio WHILE
        meio = (inicio + fim)/2;
        if(lista[meio].getTemperatura() == x)
        {
           MyIO.println("SIM");
           break;
        }
