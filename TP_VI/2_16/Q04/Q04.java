...
class Lista 
{
  ...
  {
	quicksort(primeiro.prox, ultimo, (tamanho()/2));
  }
   ...
   ************************************************/
  private void quicksort(Celula esq, Celula dir, int n)
  {
	Celula i = esq, j = dir;
	Celula pivo = acharPivo(esq, n);
	while (i != null && j != null)
	{
	   while((i != pivo || i.ant != pivo.prox) && pegaAno(i.elemento) < pegaAno(pivo.elemento) ) i = i.prox;
	   while((j != pivo || j.prox != pivo.ant) && pegaAno(j.elemento) > pegaAno(pivo.elemento)) j = j.ant;
	   if(i.ant != j.prox || i != j)
	   {
		 swap(i, j);
		 i = i.prox;
	...
	if (esq == j || esq.ant == j.prox)  quicksort(esq, j, n/2);
	if (i == dir || dir.prox == i.ant)  quicksort(i, dir, n/2);
  }

  public Celula acharPivo(Celula inicio, int a)
  {
	Celula resp;
	int b = 0;
	for(resp = inicio; b < a; resp = resp.prox, b++); 
	return resp;
  } 
 
  public void swap(Celula i, Celula j)
  {
	Pessoa tmp = i.elemento;
	i.elemento = j.elemento; 
	j.elemento = tmp;
  }

  public int pegaAno(Pessoa p)
  {
	int anoa = 0;

	if(p.getDataNasc().contains("a.C."))
      {
         if(p.getDataNasc().contains("ca."))
         {
            String vetor[] = p.getDataNasc().split(" ");
      ...
      else if(p.getDataNasc().contains("conhecida"))
         anoa = 00;   
      else{
         String vetor[] = p.getDataNasc().split(" ");
