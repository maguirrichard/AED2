...
class Lista 
{
   ...
   *********************************************/
  public void ordenarBolha(int n) 
  {
	for (int i = (n - 1); i > 0; i--) 
	{
	   for (int j = 0; j < i; j++) 
		 if (getLocalNasc1(j, j+1) > 0) swap(j, j+1); //array[j] > array[j + 1]
	}

  }

  public int getLocalNasc1(int a, int b)
  {

	int x = array[a].getLocalNasc().compareTo(array[b].getLocalNasc());

	//x < 0 caso primeiro maior que segundo
	//x == 0 caso sejam iguais
	//x > 0 caso segundo meior que primeiro
