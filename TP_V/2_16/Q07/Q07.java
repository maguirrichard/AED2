...
class Lista 
{
   ...
   **************************************/
  public void ordenarHeapsort(int n)
  {
  ...
  {
	for(int i = tamHeap; i > 1 && verificaData(i, (i/2)) == true; i /= 2) //array[i] > array[i/2]
	   swap(i, i/2);
  }
  ...
  public void reconstroi(int tamHeap)
  {
	...
	while(i <= (tamHeap/2))
	{
	   if(verificaData((2*i), (2*i+1)) == true || 2*i == tamHeap) //array[2*i] > array[2*i+1]
		 filho = 2*i;
	   else 
		 filho = 2*i + 1;
 
	   if(verificaData(i, filho) == false) //array[i] < array[filho]
	   {
		 swap(i, filho);
		 i = filho;
	   }else
...
!array[k].getDataNasc().contains("desconhecida") && !array[k].getDataNasc().contains("desconhecida"))
  ...
  public boolean compareData(int a, int b)
  {
	...
	int diaa = Integer.parseInt(arraya[0]);
	int diab = Integer.parseInt(arrayb[0]);
