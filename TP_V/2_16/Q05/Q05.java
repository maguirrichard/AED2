...
class Lista 
{
   ...
   **************************************/
  public void ordenarInsercao(int n)
  {
	for (int i = 1; i < n; i++) 
	{
	   Pessoa tmp = array[i];
	   ...
	   while ((j >= 0) && (compareNome(j, tmp) == true)) //array[j].getNome() > tmp.getNome()
	   {
		 array[j + 1] = array[j];
		 j--;
	   }
  ...
  public boolean compareNome(int j, Pessoa tmp)
  {
	boolean resp = false;

	int x = array[j].getNome().compareTo(tmp.getNome());
