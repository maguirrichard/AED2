...
class Lista 
{
  ...
  /*********************************************
   * Algoritmo de ordenacao Radix.
   * @param int n final do array a ser ordenado.
   *********************************************/
  public void ordenarRadix(int n) 
  {
	int i, m = getAnoMorte(0), exp = 1;
	Pessoa[] b = new Pessoa[n];
	...
	while(m / exp > 0)
	{
	   ...
	   for(i = 0; i < n; i++) bucket[(getAnoMorte(i) / exp) % 10]++;

	   for(i = 1; i < 10; i++) bucket[i] += bucket[i - 1];

	   for(i = n - 1; i >= 0; i--) b[--bucket[(getAnoMorte(i) / exp) % 10]] = array[i];
  ...
  public int getAnoMorte(int a)
  {
	int ano;

	if(array[a].getDataMorte().contains("vivo"))
	   ano = 1000;
	else if(array[a].getDataMorte().contains("a.C."))
	{
	   if(array[a].getDataMorte().contains("ca."))
	   {
		 String vetor[] = array[a].getDataMorte().split(" ");
